...
class Basket
{
	...
	*/
...

	{
		...
		if ($this->has($product)) {
			...
			$quantity = $this->get($product)['quantity']+1;
	...
	*/
	...
	{
		$product_id=$this->get($product)['product_id'];
		$this->storage->remove($product_id);
	}
	...
	*/
	...
	{
		$itemcount=$itemcount=count($this->storage->all());
		$ids=[];
		...
		if(count($ids)>=1){
			...
			foreach($products as $product) {
				...
				if( $product->quantity >=1){
					...
					$total += $product->getTotal(true)*$product->quantity;
		}else{
			$total += $product->getTotal(true);
		}
		}
